...
describe('checkBill', () => {
  it('turns the three fields into priced lines', () => {
    const r = checkBill({ customerName: '', customerPhone: '', gstRateBp: null, lines: [line('AMC visit', '1', '3,500'), line('Fan', '2', '1350')] }, opts);
  ...
  it('ignores a line nobody touched, but not a half-filled one', () => {
    const ok = checkBill({ customerName: '', customerPhone: '', gstRateBp: null, lines: [line('Visit', '1', '500'), blankLine('x')] }, opts);
    expect(ok.ok).toBe(true);
    const half = checkBill({ customerName: '', customerPhone: '', gstRateBp: null, lines: [line('Visit', '1', '')] }, opts);
  ...
  it('needs at least one thing done', () => {
    expect(checkBill({ customerName: '', customerPhone: '', gstRateBp: null, lines: [blankLine('a')] }, opts)).toMatchObject({
      ok: false,
      problem: { field: 'lines' },
  ...
  it('asks the new customer for a name', () => {
    expect(checkBill({ customerName: '  ', customerPhone: '', gstRateBp: null, lines: [line('Visit', '1', '500')] }, { ...opts, needsCustomerName: true })).toMatchObject({
      ok: false,
  ...
  it('refuses a quantity of zero or a rate that is not a number', () => {
    expect(checkBill({ customerName: '', customerPhone: '', gstRateBp: null, lines: [line('Visit', '0', '500')] }, opts)).toMatchObject({ problem: { field: 'qty' } });
    expect(checkBill({ customerName: '', customerPhone: '', gstRateBp: null, lines: [line('Visit', '1', 'five')] }, opts)).toMatchObject({ problem: { field: 'rate' } });
  });

  it('puts one GST rate on every line for a registered owner, and knows when none was chosen', () => {
    const chosen = checkBill({ customerName: '', customerPhone: '', gstRateBp: 1800, lines: [line('Visit', '1', '500')] }, { ...opts, chargesGst: true });
    expect(chosen.ok && chosen.lines[0]).toMatchObject({ taxRateBp: 1800, taxRateChosen: true });
    const none = checkBill({ customerName: '', customerPhone: '', gstRateBp: null, lines: [line('Visit', '1', '500')] }, { ...opts, chargesGst: true });
